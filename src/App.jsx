...
const TimetableView = ({ role = 'director', addNotification = () => {}, cells = {}, setCells = () => {}, saveCellToDB = async () => {}, teachers = [], classrooms = [], batches = [], timeSlots = [], currentTime = '08:00' }) => {
  ...
  const saveAssign = async () => {
    ...
    setCells({ ...cells, [assignSlot]: newCellData });
    // Save to Supabase
    if (saveCellToDB) await saveCellToDB(assignSlot, newCellData);
  ...
  const clearSlot = async () => {
    ...
    if (saveCellToDB) await saveCellToDB(assignSlot, null);
      ...
      <div className="flex min-h-screen">
        ...
        <main className="flex-1 overflow-x-hidden min-w-0">
          ...
          <div className="p-4 md:p-8 max-w-[1600px]">
            ...
            {activeView === 'timetable' && <TimetableView role={role} addNotification={addNotification} cells={cells} setCells={setCells} saveCellToDB={saveCellToDB} teachers={teachers} classrooms={classrooms} batches={batches} timeSlots={timeSlots} currentTime={currentTime} />}
...
export default function App() {
  ...
  // SAVE INDIVIDUAL CELL — used by Manager's assign modal
  const saveCellToDB = async (cellKey, cellData) => {
    const timeSlot = cellKey.split('-')[0];
    const batch = cellKey.slice(timeSlot.length + 1);
    if (!cellData) {
      // DELETE cell
      const { error } = await supabase.from('cells').delete().eq('cell_key', cellKey);
  ...
  const updateRoomInDB = async (id, updates) => {
    ...
    if (error) alert('Failed to update room: ' + error.message);
  };
  ...
  const updateBatchInDB = async (id, updates) => {
    ...
    if (error) alert('Failed to update batch: ' + error.message);
  };
